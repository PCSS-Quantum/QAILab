...
def circuit_formatter(
    problem: CircuitProblem,
    parameters: Mapping[Parameter, Iterable | complex] | Iterable | None = None,
    auto_bind: bool = True,
    ...
    """
    if auto_bind:
        circuit = _bind_params(problem, parameters, initial_state)
        return [circuit]
...
def _bind_params(
    problem: CircuitProblem,
    parameters: Mapping[Parameter, Iterable] | Iterable | None,
    initial_state: Statevector | Sequence[complex] | str | int | None
) -> QuantumCircuit:
    """ Binding parameters """
