...
try:
	...
	class HybridOptimizer:
		...
		def __init__(
			self,
			model,  # noqa: ANN001
			lr_classical=0.01,  # noqa: ANN001
			lr_quantum=0.01,  # noqa: ANN001
			optimizer_quantum='SGD',  # noqa: ANN001
			optimizer_classical='Adam',  # noqa: ANN001
			betas=(0.9, 0.999),  # noqa: ANN001
			spsa_resamplings=1,  # noqa: ANN001
			spsa_gamma_decay=0.101,  # noqa: ANN001
			spsa_alpha_decay=0.602,  # noqa: ANN001
		):
			pass


AVAILABLE_OPTIMIZERS: dict[str, type[Optimizer] | type[HybridOptimizer]] = {
	opt.__name__.lower(): opt
	for opt in [
		optim.Adam,
		optim.AdamW,
		optim.SGD,
...
class QModel(BaseEstimator):
	...
	def __init__(
		...
		if isinstance(optimizer_type, str):
			if optimizer_type not in AVAILABLE_OPTIMIZERS:
				raise ValueError(f'Unknown optimizer: {optimizer_type}. Available optimizers are: {list(AVAILABLE_OPTIMIZERS.keys())}')
		...
		if self.optimizer_type == HybridOptimizer:
			if self.quantum_learning_rate != 'auto' and self.learning_rate != 'auto':
				self.optimizer = self.optimizer_type(self.module, lr_classical=self.learning_rate, lr_quantum=self.quantum_learning_rate)  # type: ignore
	...
	def _x_y_to_tensor(
		self, x: Tensor | np.ndarray | pd.DataFrame, y: Tensor | np.ndarray | pd.DataFrame | pd.Series
	) -> tuple[Tensor, Tensor]:
		x = self._x_to_tensor(x)
		if isinstance(y, np.ndarray):
			y = torch.tensor(y, dtype=torch.int64) if y.dtype.kind == 'i' else torch.tensor(y, dtype=torch.float32)
		elif isinstance(y, pd.DataFrame):
			y = torch.tensor(y.values, dtype=torch.float32)
		elif isinstance(y, pd.Series):
			y = torch.tensor(y.values, dtype=torch.int64) if y.dtype == np.dtype('int64') else torch.tensor(y.values, dtype=torch.float32)
	...
	def _accuracy(
		...
		y_pred = torch.argmax(y_pred, dim=1) if y_pred.dim() == 2 else torch.where(y_pred > 0.5, 1.0, 0.0)
	...
	def _train_loop(self, train_loader: DataLoader, validation_loader: DataLoader, epochs: int) -> None:
		self.loss_history = {'training': [], 'validation': []}
	...
	def set_params(self, **params) -> 'QModel':
		...
		**params: dict
		...
		def _update_optimizer() -> None:
			if self.optimizer_type == HybridOptimizer:
				if self.quantum_learning_rate != 'auto' and self.learning_rate != 'auto':
					self.optimizer = self.optimizer_type(
						self.module,
						lr_classical=self.learning_rate,  # type: ignore
		...
		for key, value in params.items():
			if key not in valid_params:
				raise ValueError(f'Invalid parameter {key!r} for estimator {self}. Valid parameters are: {valid_params.keys()!r}.')
			if key == 'device':
				self.device = value
