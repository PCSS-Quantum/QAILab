...
from qailab.circuit.utils import assign_input_weight, filter_params
...
class ExpVQCFunction(Function):  # pylint: disable=abstract-method
	...
	def _forward_single(
		...
		params = assign_input_weight(launcher_forward.problem.instance, fn_in_numpy, weight_numpy)
		res = launcher_forward.run(parameters=params)
		arr = distribution_to_array(res.distribution)
		return torch.tensor(arr, dtype=fn_in.dtype, requires_grad=True).to(fn_in.device)
	...
	def setup_context(
		ctx: Any,  # noqa: ANN401
		inputs: tuple[torch.Tensor, torch.Tensor, QLauncher, QLauncher],
		output: torch.Tensor,
	) -> None:
		"""
	...
	def _backward_single(
		fn_in: torch.Tensor, weight: torch.Tensor, launcher_backward: QLauncher, grad_output: torch.Tensor
	) -> tuple[torch.Tensor, torch.Tensor]:
		fn_in_numpy = fn_in.detach().numpy()
		weight_numpy = weight.detach().numpy()

		params = assign_input_weight(launcher_backward.problem.instance, fn_in_numpy, weight_numpy)

		res = launcher_backward.run(parameters=params)
		...
		return (
			...
			if len(res.result['weight']) > 0
			else torch.tensor([], dtype=weight.dtype).to(fn_in.device),
		)

	@staticmethod
	@once_differentiable
	def backward(  # pylint: disable=arguments-differ # type: ignore
		ctx: Any,  # noqa: ANN401
		...
		for in_single, grad_single in zip(fn_in, grad_output, strict=True):
...
class ExpVQCFunctionMP(Function):  # pylint: disable=abstract-method
	...
	@staticmethod
	def _forward_single(fn_in: torch.Tensor, weight: torch.Tensor, launcher_forward: QLauncher, aql_instance: AQL) -> None:
	...
	def forward(  # pylint: disable=arguments-differ
		...
		with AQL('default') as aql_instance:
			...
			for res in aql_instance.results():
				if res is None:
					raise ValueError('QLauncher result returned None!')
				arr = distribution_to_array(res.distribution)
	...
	def setup_context(
		ctx: Any,  # noqa: ANN401
		inputs: tuple[torch.Tensor, torch.Tensor, list[QLauncher], list[QLauncher]],
		output: torch.Tensor,
	) -> None:
		"""
		Called after forward, saves args from forward to be later used in backward.
	...
	@staticmethod
	def _backward_single(fn_in: torch.Tensor, weight: torch.Tensor, launcher_backward: QLauncher, aql_instance: AQL) -> None:
		fn_in_numpy = fn_in.cpu().detach().numpy()
		weight_numpy = weight.cpu().detach().numpy()

		params = assign_input_weight(launcher_backward.problem.instance, fn_in_numpy, weight_numpy)

		aql_instance.add_task(launcher_backward, parameters=params)

	@staticmethod
	@once_differentiable
	def backward(  # pylint: disable=arguments-differ, too-many-locals # type: ignore
		ctx: Any,  # noqa: ANN401
		grad_output: torch.Tensor,
	) -> tuple[torch.Tensor, torch.Tensor, None, None]:
		"""
		...
		with AQL('default') as aql_instance:
			...
			for i, (in_single) in enumerate(fn_in):
				ExpVQCFunctionMP._backward_single(in_single, weight, launchers_backward[i % len(launchers_backward)], aql_instance)

			aql_instance.start()
			results = aql_instance.results()

			for res, out_grad in zip(results, grad_output, strict=True):
				if res is None:
					raise ValueError('QLauncher result returned None!')
				...
				weight_grads.append(wgrad * np.pi @ out_grad)
...
class ArgMax(Function):  # pylint: disable=abstract-method
	...
	@staticmethod
	def forward(fn_in: torch.Tensor) -> torch.Tensor:  # pylint: disable=arguments-differ
		"""
		Forward run.
