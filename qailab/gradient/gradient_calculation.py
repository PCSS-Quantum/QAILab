...
def _param_grads_to_jacobian(grads: list[dict[int, float]], num_possible_values: int) -> np.ndarray:
...
def calculate_jacobian(
	...
	grads_amp = dict(zip(set_params_amp.keys(), grads_amp, strict=True))
	grads_compat = dict(zip(set_params_compatible.keys(), grads_compat, strict=True))

	grads = grads_amp | grads_compat

	return _param_grads_to_jacobian([grads[k] for k in set_params], num_possible_values)
...
def calculate_gradients_compatible(
	...
	Returns:
		list[dict[int, float]]: List of gradients for each of the parameters in `set_params`.
	"""
	gradient_type = {
	...
	if len(set_params) == 0:
		return []
	params, values = zip(*list(set_params.items()), strict=True)

	return gradient_calc.run([circuit], [values], [params], shots=shots).result().gradients[0]
...
def calculate_gradients_incompatible(
	...
	for param in set_params:
		distribution = (
			backend.samplerV1.run(circuit.assign_parameters(set_params | {param: set_params[param] + epsilon}), shots=shots)
			.result()
			.quasi_dists[0]
		)
