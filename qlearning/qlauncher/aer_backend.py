    ...
    def run_wrapper(pubs, *args, shots: int | None = None):
        ...
        for pub in pubs:
            if isinstance(pub, QuantumCircuit):
                pub = transpile(pub.decompose(), sampler._backend, optimization_level=0)  # pylint: disable=protected-access
            elif isinstance(pub, tuple):
                pub = (transpile(pub[0].decompose(), sampler._backend, optimization_level=0), *pub[1:])  # pylint: disable=protected-access
            newpubs.append(pub)
        ...
        for pub in pubs:
            if isinstance(pub, tuple):
                pub = (
                    transpile(
                        pub[0].decompose(),
                        estimator._backend,  # pylint: disable=protected-access
